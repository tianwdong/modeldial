...
本地修复候选已提升为 `preview.12`／Build 111：unsigned ad-hoc 路径不再启用 hardened runtime，Developer ID 路径继续保留 hardened runtime／timestamp；新增门禁逐个验证 App 内全部 Mach-O 的 ad-hoc、无 runtime、无证书 Authority、无 Team ID 和严格签名。该候选已通过源码、构建和冻结后端验证并进入本地提交阶段，但尚未推送、正式打包或发布。历史公开源码根仍保持单一无父 `main`；正式签名二进制、stable appcast、正式 stable Homebrew Cask 和干净机器 Gatekeeper 验收仍未完成。
...
- 2026-08-10：第三方正常 Mac 复现 `preview.11` 冻结 Python 与 Sparkle 嵌套组件的 Library Validation／Team ID 不一致启动失败；源码修复候选提升到 `preview.12`／Build 111。ad-hoc 路径移除 hardened runtime，Developer ID 分支保持原策略；新增逐 Mach-O 签名门禁、后端 loading／available／unavailable 状态、错误诊断分层、Radar／设置／模型接入失效态、非流式桥接双管道并发排空与 120／330 秒终止边界。实际构建先抓到并修复系统 Bash 3.2 空数组展开问题，最终 `build/modeldial-candidate.app` 回读 `0.1.0（Build 111）` 和官方 Radar URL，60 个 Mach-O 均为 ad-hoc、无 hardened runtime、无 Authority／Team ID 且严格签名有效。冻结后端在全新隔离缓存从官方源取得 `http/refreshed`／`fresh`、8 份快照、最新 `snapshot-2026-08-10T06-00-00Z` 和 15 行榜单；一次真实 TLS 瞬时失败促成仅对 `unavailable` 的单次整包重试。定向回归 `156/156`、最终全量 Python／Swift 合同 `1442/1442`（`857.252s`）、架构基线 `11/11`、完整 `./build.sh` 和 `git diff --check` 通过。Tap README 的本地版本／hash 漂移已校正；源码修复已进入本地提交阶段，但尚未 push、tag、package、Release、上传、appcast／官网／Cask 远端变更或 `/Applications` 覆盖。
