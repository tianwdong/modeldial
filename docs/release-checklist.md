...
- [x] 运行 Python 全量回归和完整 App 构建；2026-08-08 当前未提交候选为 `1404/1404`，完整 `./build.sh`、macOS 13 bundle 兼容门禁和严格签名通过。
...
- [ ] 创建公开 GitHub 仓库、添加 `origin`、推送 `main`，并核对本地／远端 SHA。
- [ ] 配置仓库描述、Topics 和默认分支保护。
...
- [x] 保留 `build.sh` 作为本地统一入口；Python 3.14.3 installer 由独立 lock 固定 URL、SHA-256 和 signer，只解包到 `build/`，PyInstaller、certifi CA bundle 及传递构建依赖使用 exact-version requirements；现有 venv 失配时只重建隔离的 `build/pyinstaller-env`。
...
- [ ] 为正式构建依赖补充 artifact hashes／来源证明；Python installer 已固定 SHA-256 和 signer，但 PyPI requirements 仍只有精确版本，不能替代完整供应链校验。
...
- [ ] 补齐 Python、PyInstaller、OpenSSL、xz、zstd、mpdecimal、Sparkle 等最终二进制依赖的许可证、Notice 和 SBOM。
