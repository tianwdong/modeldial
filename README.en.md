...
> This repository is an auditable public-source candidate. No formal download package is available yet; source builds use ad-hoc signing by default.

## Website and formal installation

- Product website: <https://modeldial.com>
- The first formal DMG has not been released yet. Once published, download it from GitHub Releases, open the DMG, drag ModelDial to Applications, then launch it from Applications for the first run.
- The first formal release supports macOS 13+ on Apple Silicon; Intel Macs are not currently supported.
...
Source builds target macOS 13+ on Apple Silicon and require Xcode 16.4; no separate Python runtime installation is required. On the first `build.sh` run, [`python-runtime.lock.json`](build-support/python-runtime.lock.json) downloads the official python.org universal2 Python 3.14.3 installer, verifies its pinned SHA-256 and Python Software Foundation installer signature, and only extracts it under the ignored `build/` directory. It never performs a system installation or falls back to Homebrew or a Python found on `PATH`. The script then pins PyInstaller 6.21.0, the certifi CA bundle, and their build dependencies in `build/pyinstaller-env` from [`pyinstaller-requirements.txt`](build-support/pyinstaller-requirements.txt). Before signing, it exercises TLS, SHA-256, and zstd, requires a non-empty CA store, and recursively rejects Mach-O files whose minimum macOS version exceeds the App declaration or whose dynamic libraries use non-system absolute paths. These gates do not replace clean-machine acceptance on macOS 13 and 14; PyPI artifact hashes, the formal SBOM, and the complete license inventory remain separate release gates. SwiftPM also downloads the pinned Sparkle 2.9.4 dependency from `Package.resolved` on the first build.
...
The open-source build uses ad-hoc signing by default. To use an installed signing identity:
