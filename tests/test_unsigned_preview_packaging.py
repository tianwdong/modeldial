...
class UnsignedPreviewPackagingTest(unittest.TestCase):
    ...
    def test_preview_injects_and_reads_back_official_reference_feed(self) -> None:
        ...
        self.assertIn(
            "preview.1|preview.2|preview.3|preview.4|preview.5|preview.6|preview.7|preview.8|preview.9|preview.10|preview.11|preview.12|preview.13|preview.14|preview.15|preview.16|preview.17)",
            self.source,
        )
        self.assertIn(
    ...
    def test_artifacts_are_versioned_arm64_dmg_zip_and_checksums(self) -> None:
        self.assertIn('PREVIEW_LABEL="${MODELDIAL_PREVIEW_LABEL:-preview.17}"', self.source)
